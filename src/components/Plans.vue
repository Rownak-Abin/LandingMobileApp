<script setup>

const site = "https://preview.colorlib.com/theme/nexus/#";

</script>
<template>
     <section class="price_area p_120">
    <div class="container">
      <div class="main_title">
        <h2>Choose Your Price Plan</h2>
        <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore
          magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation.</p>
      </div>
      <div class="price_inner row">
        <div class="col-lg-4">
          <div class="price_item">
            <div class="price_head">
              <h4>Real Basic</h4>
            </div>
            <div class="price_body">
              <ul class="list">
                <li><a :href="site">2.5 GB Space</a></li>
                <li><a :href="site">Secure Online Transfer</a></li>
                <li><a :href="site">Unlimited Styles</a></li>
                <li><a :href="site">Customer Service</a></li>
              </ul>
            </div>
            <div class="price_footer">
              <h3><span class="dlr">$</span> 39 <span class="month">Per <br>Month</span></h3>
              <a class="main_btn" :href="site">Get Started</a>
            </div>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="price_item">
            <div class="price_head">
              <h4>Real Standard</h4>
            </div>
            <div class="price_body">
              <ul class="list">
                <li><a :href="site">10 GB Space</a></li>
                <li><a :href="site">Secure Online Transfer</a></li>
                <li><a :href="site">Unlimited Styles</a></li>
                <li><a :href="site">Customer Service</a></li>
              </ul>
            </div>
            <div class="price_footer">
              <h3><span class="dlr">$</span> 69 <span class="month">Per <br>Month</span></h3>
              <a class="main_btn" :href="site">Get Started</a>
            </div>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="price_item">
            <div class="price_head">
              <h4>Real Ultimate</h4>
            </div>
            <div class="price_body">
              <ul class="list">
                <li><a :href="site">Unlimited Space</a></li>
                <li><a :href="site">Secure Online Transfer</a></li>
                <li><a :href="site">Unlimited Styles</a></li>
                <li><a :href="site">Customer Service</a></li>
              </ul>
            </div>
            <div class="price_footer">
              <h3><span class="dlr">$</span> 99 <span class="month">Per <br>Month</span></h3>
              <a class="main_btn" :href="site">Get Started</a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<style>
    
</style>